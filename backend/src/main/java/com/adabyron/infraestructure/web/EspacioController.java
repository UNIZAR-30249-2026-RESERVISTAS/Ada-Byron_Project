...
public class EspacioController {
        ...
        @Operation(summary = "Obtener información de un espacio", description = "Devuelve el identificador, la categoría, el número de ocupantes máximos, el tamaño, si es reservable "
                        +
                        "y el horario disponible del espacio cuyo ID coincide con el proporcionado")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Espacio encontrado.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
        ...
        @Operation(summary = "Cambiar la categoría de un espacio", description = "Sustituye la categoría actual del espacio indicado por la categoría especificada en el cuerpo de la petición.")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Categoría actualizada correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
                        @ApiResponse(responseCode = "404", description = "No existe ningún espacio con el ID indicado.", content = @Content)
        })
        @PutMapping("/{id}/categoria")
        public EspacioDTO cambiarCategoria(@PathVariable String id, @RequestBody CambiarCategoriaDTO dto)
                        throws TimeoutException {
                // return EspacioDTO.fromEntity(espacioService.cambiarCategoria(id,
                // Categoria.desdeNombre(dto.categoria())));
        ...
        @Operation(summary = "Cambiar el estado de un espacio", description = "Sustituye el estado actual del espacio indicado por el estado especificado en el cuerpo de la petición.")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Estado actualizado correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
                        @ApiResponse(responseCode = "404", description = "No existe ningún espacio con el ID indicado.", content = @Content)
        })
        @PutMapping("/{id}/reservable")
        public EspacioDTO cambiarEstado(@PathVariable String id, @RequestBody CambiarReservableDTO dto)
                        throws TimeoutException {
                // return EspacioDTO.fromEntity(espacioService.cambiarReservable(id,
                // dto.reservable()));
        ...
        @Operation(summary = "Obtener el horario de un espacio", description = "REQ-C5: Devuelve el horario disponible del espacio. "
                        +
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Horario del espacio obtenido correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = HorarioDTO.class))),
                        @ApiResponse(responseCode = "404", description = "No existe ningún espacio con el ID indicado.", content = @Content)
        })
        ...
        @Operation(summary = "Cambiar el horario de un espacio", description = "REQ-C6: Permite a los gerentes cambiar el horario de reserva de un espacio. "
                        +
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Horario actualizado correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
        ...
        public EspacioDTO cambiarHorario(
                        @Parameter(description = "ID del espacio", example = "107", required = true) @PathVariable String id,
                        @Parameter(description = "UUID del gerente que ejecuta la operación", example = "123e4567-e89b-12d3-a456-426614174000", required = true) @RequestParam UUID gerenteId,
                        @RequestBody CambiarHorarioDTO dto) throws TimeoutException {
                HorarioDisponible nuevoHorario = new HorarioDisponible(dto.horaApertura(), dto.horaCierre());
                // return EspacioDTO.fromEntity(espacioService.cambiarHorario(id, nuevoHorario,
                // gerenteId));
        ...
        @Operation(summary = "Restablecer el horario de un espacio al horario del edificio", description = "REQ-C6: Elimina el horario específico del espacio y restablece el horario por defecto del edificio Ada Byron (8:00 - 21:00). "
                        +
                        "Solo puede realizarse por un usuario con rol GERENTE.")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Horario restablecido al del edificio correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
                        @ApiResponse(responseCode = "403", description = "El usuario no tiene rol de GERENTE para realizar esta operación.", content = @Content),
                        @ApiResponse(responseCode = "404", description = "No existe ningún espacio con el ID indicado o la persona no existe.", content = @Content)
        })
        ...
        public EspacioDTO restablecerHorario(
                        ...
                        @Parameter(description = "UUID del gerente que ejecuta la operación", example = "123e4567-e89b-12d3-a456-426614174000", required = true) @RequestParam UUID gerenteId)
                        throws TimeoutException {
                // return EspacioDTO.fromEntity(espacioService.restablecerHorario(id,
                // gerenteId));
        ...
        @Operation(summary = "Obtener la asignación actual de un espacio", description = "Devuelve a quién o qué está asignado el espacio: la EINA, un departamento o una o más personas concretas (REQ-C7, REQ-C8).")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Asignación del espacio obtenida correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = AsignacionDTO.class))),
                        @ApiResponse(responseCode = "404", description = "No existe ningún espacio con el ID indicado.", content = @Content)
        })
        ...
        @Operation(summary = "Asignar un espacio a la EINA", description = "Asigna el espacio indicado a la EINA. " +
                        "Aplicable a aulas y salas comunes (REQ-C9).")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Espacio asignado a la EINA correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
        ...
        @Operation(summary = "Asignar un espacio a un departamento", description = "Asigna el espacio indicado al departamento especificado. "
                        +
                        "Aplicable a despachos, seminarios y laboratorios (REQ-C10, REQ-C11).")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Espacio asignado al departamento correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
        ...
        public EspacioDTO asignarADepartamento(
                        ...
                        @RequestBody AsignarDepartamentoDTO dto) throws TimeoutException {
                // return EspacioDTO.fromEntity(espacioService.asignarADepartamento(id,
                // dto.departamentoId()));
        ...
        @Operation(summary = "Asignar un espacio a una o más personas", description = "Asigna el espacio indicado a las personas especificadas. "
                        +
                        "Solo aplicable a despachos, y las personas deben tener rol de " +
                        "investigador contratado o docente-investigador (REQ-C10).")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Espacio asignado a las personas correctamente.", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = EspacioDTO.class))),
        ...
        public EspacioDTO asignarAPersonas(
                        @PathVariable String id,
                        @RequestBody AsignarPersonasDTO dto) throws TimeoutException {
                // return EspacioDTO.fromEntity(espacioService.asignarAPersonas(id,
                // dto.personaIds()));
        ...
        public ResponseEntity<List<String>> filtrarPorAforo(@RequestParam("personas") int personasBuscadas)
                        throws TimeoutException {
